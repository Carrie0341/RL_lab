...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    if os.path.exists(isaacsim_vscode_filename):
        ...
        path_names = ['"${workspaceFolder}/' + rel_path.replace('\\', '/') + "/" + path_name + '"' for path_name in path_names]
    else:
        path_names = []
        print(
...
def overwrite_default_python_interpreter(isaaclab_settings: str) -> str:
    ...
    python_exe = python_exe.replace("\\", "/")  # 修正Windows路徑
...
def main():
    ...
    isaaclab_python_env_filename = os.path.join(PROJECT_DIR, ".vscode", ".python.env")
    with open(isaaclab_python_env_filename, "w") as f:
        f.write(f"PYTHONPATH={ISAACSIM_DIR}\n")
        f.write(f"CONDA_PYTHON={conda_python_path}\n")
        f.write(f"ISAAC_PATH={ISAACSIM_DIR}\n")
